...
pub async fn run_daemon(options: DaemonOptions) -> Result<()> {
    ...
    let runtime = TaskRuntime::new(config, &options.log_dir)?.with_event_log_path(event_log_path);

    let mut daemon_api = DaemonApi::new(runtime).with_applied_config_path(applied_config_path);
    if let Some(token) = options.auth_token {
        daemon_api = daemon_api.with_auth_token(token);
    }
    let api = Arc::new(Mutex::new(daemon_api));
    spawn_maintenance_loop(api.clone());
    spawn_startup_tasks(api.clone());
    #[cfg(unix)]
...
fn spawn_startup_tasks(api: Arc<Mutex<DaemonApi>>) {
    tokio::spawn(async move {
        let _ = api.lock().await.start_startup_tasks().await;
    });
}
